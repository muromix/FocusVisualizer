...
| **macOS** | **focus_backend** | **システム最優先** | **Focus Visualizer.app** | ✅ **完全対応 (v0.9.4_Alpha)** |

**Mac環境の注意事項 (v0.9.4_Alpha〜)**:
- **スタンドアロン化**: バックエンドが `focus_backend` バイナリとして同梱されており、Pythonのインストール不要で動作。
- **配布形式 (Unified Folder)**: アプリ本体と、プラグイン・説明書をまとめたフォルダ（`Focus_Visualizer_v0.9.4_Alpha`）を並べて配布。
...
- Mac版の場合、DMG内には以下の構成を配置する：
  ...
  2.  `Focus_Visualizer_v0.9.4_Alpha` (関連ファイルフォルダ)
